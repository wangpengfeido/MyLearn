...
const qnameCapture = `((?:${ncname}\\:)?${ncname})`       // qname。一开始对这个冒号表示奇怪，经查询后冒号及前面的名字是xml中表示命名空间的。
...
export function parseHTML(html, options) {
  //
  const stack = []    // 记录已解析的非自闭开始标签，在与结束标签匹配时使用。   {tag: 标签名, lowerCasedTag: 小写标签名, attrs: 属性数组, start: 开始位置, end: 结束位置}
  const expectHTML = options.expectHTML
  const isUnaryTag = options.isUnaryTag || no
  const canBeLeftOpenTag = options.canBeLeftOpenTag || no
  let index = 0    // 当前已解析到的位置。随着解析的进行，解析位置会增长，html也会随之被截取
  let last, lastTag    // last是上次解析开始时的html。lastTag时上次解析的标签名。（这里的一次解析指下面的一次循环）
  while (html) {
    // 保留html副本
    last = html
  ...
  function parseStartTag() {
    ...
    if (start) {
      ...
      const match = {
        tagName: start[1],    // 标签名
        attrs: [],    // 属性名数组   数组元素为{match,start:属性开始位置,end:属性结束位置}
        start: index    // 标签起点在html中的位置
      }
      // 解析位置增加，表示标签开头已解析完
      ...
      // 如果到了开始标签的结尾
      if (end) {
        match.unarySlash = end[1]    // 表示该标签是否是一个自闭标签。如果是这个值是"/"
        // 解析位置增加，表示标签结尾已解析完
        advance(end[0].length)
        match.end = index    // 标签终点在html中的位置
        // 返回解析出的信息
        return match
      }
    }
  ...
  function handleStartTag(match) {
    ...
    const l = match.attrs.length    // 属性数组的长度
    const attrs = new Array(l)    // 构建一个和属性数组长度相同的数组
    // 循环处理属性
    for (let i = 0; i < l; i++) {
      const args = match.attrs[i]    // 属性
      const value = args[3] || args[4] || args[5] || ''    // 属性值。（参考属性匹配正则）
