...
function genStaticKeys (keys: string): Function {
  return makeMap(
    // 这些是所有ASTElement都拥有的固有属性，所以不做检查
...
function markStatic (node: ASTNode) {
  ...
  if (node.type === 1) {
    ...
    // 2.静态slot内容不能用于热重载
    if (
      !isPlatformReservedTag(node.tag) && // 不是保留标签 (是component)
      !node.component && // 没有使用is绑定动态组件（这里暂时是猜测）
      node.tag !== 'slot' && // 不是 slot 标签
      node.attrsMap['inline-template'] == null // 没有使用 inline-template 属性
    ) {
...
function isStatic (node: ASTNode): boolean {
  ...
  return !!(node.pre/* 有v-pre指令 */ || (
    ...
    !node.if && !node.for && // 没有 v-if 与 v-for 与 v-else
    !isBuiltInTag(node.tag) && // 不是内置标签(solt,component)
    isPlatformReservedTag(node.tag) && // 不是 component （是平台保留标签）
